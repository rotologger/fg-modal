...
class FGModal {
  ...
  events() {
    this.modals?.forEach((modal, index) => {
      ...
      if (selector) {
        document.querySelectorAll(selector)?.forEach(el => el.addEventListener("click", () => this.show(modal)));
      }
      ...
      document.addEventListener("keydown", e => {
        if (e.key === "Escape" && !modal.classList.contains("hidden")) this.hide(modal, index);
      });
    });
  }
  show(modal, index = "") {
  ...
  hide(modal, index) {
    ...
    if (setCookie) this.createCookie(`seenModal-${index}`, "true", expiresAfter);
  }
